  ...
  <div class="max-w-3xl mx-auto p-6">
    ...
    <div class="mb-4">
      @if($article->cover_url)
        <img src="{{ $article->cover_url }}" alt="cover" class="w-full max-w-md rounded shadow" />
      @endif
    </div>

    <form action="{{ route('admin.articles.update', $article) }}" method="post" enctype="multipart/form-data" class="bg-white p-6 rounded-xl shadow space-y-5">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <input name="excerpt" value="{{ old('excerpt', $article->excerpt) }}" class="w-full border rounded px-3 py-2">
      </div>

      <div>
        <label class="block text-sm font-medium mb-1">Konten (HTML diperbolehkan)</label>
        <textarea name="content" rows="10" class="w-full border rounded px-3 py-2">{{ old('content', $article->content) }}</textarea>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="datetime-local" name="published_at" class="w-full border rounded px-3 py-2" value="{{ old('published_at', optional($article->published_at)->format('Y-m-d\TH:i')) }}">
        </div>
      </div>
